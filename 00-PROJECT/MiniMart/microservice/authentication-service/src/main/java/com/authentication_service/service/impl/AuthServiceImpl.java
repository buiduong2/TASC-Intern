...
public class AuthServiceImpl implements AuthService {
    ...
    public UserDTO getInfo(Long authId) {
        return repository.findByIdForAuth(authId)
                .map(mapper::toDTO)
                .orElseThrow(() -> new ResourceNotFoundException(ErrorCode.USER_NOT_FOUND.format(authId)));
    }
    ...
    public UserDTO register(RegisterReq registerReq) {
        ...
        Role customerRole = roleRepository.findByName(RoleName.CUSTOMER)
                .orElseThrow(() -> new ResourceNotFoundException(ErrorCode.ROLE_NAME_NOT_FOUND.format("CUSTOMER")));
