...
public class CriteriaApiUtils {
    ...
    public static <X, Y> Join<X, Y> getOrCreateJoin(From<?, X> root, SingularAttribute<X, Y> attr,
            JoinType type) {
    ...
    @SuppressWarnings("unchecked")
    public static <X, Y> Join<X, Y> getOrCreateJoin(From<?, X> root, ListAttribute<X, Y> attr) {
        return (Join<X, Y>) findJoin(root, attr)
                .orElseGet(() -> root.join(attr));
    }
    ...
    private static <X, Y> Optional<Join<X, ?>> findJoin(From<?, X> root, Attribute<X, Y> attr, JoinType type) {
    ...
    private static <X, Y> Optional<Join<X, ?>> findJoin(From<?, X> root, Attribute<X, Y> attr) {
        return root.getJoins()
                .stream()
